...
revoke execute on function app.my_sessions_between(date, date) from public, anon;
grant execute on function app.my_sessions_between(date, date) to authenticated;
...
language sql stable security definer set search_path = '' as $$
  select e.course_class_id, c.code, c.name
  from public.enrollments e
  join public.course_classes cc on cc.id = e.course_class_id
  join public.courses c on c.id = cc.course_id
  where e.student_id = app.current_student_id() and e.status = 'ACTIVE'
  order by c.name
$$;
revoke execute on function app.my_active_course_classes() from public, anon;
grant execute on function app.my_active_course_classes() to authenticated;
